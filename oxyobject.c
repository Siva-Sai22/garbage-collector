...
oxy_object_t *oxy_add(oxy_object_t *a, oxy_object_t *b) {
    ...
    if (a->kind == INTEGER) {
        if (b->kind == INTEGER) {
            return new_oxy_integer(a->data.v_int + b->data.v_int);
        } else if (b->kind == FLOAT) {
            return new_oxy_float(a->data.v_int + b->data.v_float);
        } else
            return NULL;
    }
    ...
    if (a->kind == FLOAT) {
        if (b->kind == FLOAT) {
            return new_oxy_float(a->data.v_float + b->data.v_float);
        } else if (b->kind == INTEGER) {
            return new_oxy_float(a->data.v_float + b->data.v_int);
        } else
            return NULL;
    }
    ...
    if (a->kind == STRING) {
        ...
        oxy_object_t *obj = new_oxy_string(new_str);
        free(new_str);
        return obj;
    }
